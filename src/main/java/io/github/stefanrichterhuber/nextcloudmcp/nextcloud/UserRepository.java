 ...
 *
 * <p>Two categories of data are managed per user:
 * <ul>
 *   <li><b>Credentials</b> — the Nextcloud login name and app password obtained via
 *       Nextcloud Login Flow V2, used to authenticate WebDAV and REST calls on behalf
 *       of the user.</li>
 *   <li><b>Access configuration</b> — an optional set of restrictions that limit which
 *       files and content types the AI is permitted to access for a given user.</li>
 * </ul>
 *
 * <p>Methods come in two variants: those that operate on an explicitly named user
 * ({@code ForUser(String name)}) and convenience overloads that resolve the name from
...
public interface UserRepository {
     ...
     *
     * @param rootFolder   optional path prefix that confines all file operations to a
     *                     specific folder subtree (e.g. {@code "/Documents"}). When
     *                     {@code null} or empty, the entire Nextcloud is accessible.
     * @param filePatterns optional set of glob patterns (e.g. {@code "*.md"},
     *                     {@code "*.txt"}) that limit which files are visible to the AI.
     ...
     * @return the credentials, or {@link Optional#empty()} if no credentials are stored
     *         for that user.
     */
    Optional<NextcloudUserCredentials> getCredentialsForUser(String name);

    /**
     * Persists Nextcloud credentials for the given user, replacing any previously stored
     * credentials.
     *
    ...
    /**
     * Persists Nextcloud credentials for the currently authenticated user, replacing any
     ...
     *
     * @return the access configuration, or {@link Optional#empty()} if no configuration
     *         has been saved for the current user (in which case no restrictions apply).
     */
    ...
    /**
     * Persists the access configuration for the given user, replacing any previously
    ...
    /**
     * Persists the access configuration for the currently authenticated user, replacing
